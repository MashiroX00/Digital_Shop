...
export default function CardItem(props) {
    ...
    const rawPath = props.item.imgUrl?.replace(/\\/g, "/"); // แปลง \ เป็น /
    const fullImageUrl = rawPath
        ? process.env.NEXT_PUBLIC_API_PICTURE_URL + rawPath
        : "/exampleItem.png";
    ...
    const checkout = "/products/checkout/" + id;
    return (
        <div className="flex flex-col bg-[var(--yale-blue)] w-fit h-full rounded-xl relative" data-aos="fade-up" data-aos-duration="1000">
            <div className="flex flex-col justify-between h-full">
                <Image src={fullImageUrl} width={350} height={350} alt={name} className="rounded-t-2xl"></Image>
                ...
                <div className="flex flex-row justify-center mx-2 ">
                    <Link href={checkout} className="bg-[var(--mikado-yellow)] my-3 mx-1 p-2 rounded-xl w-full hover:bg-[var(--oxford-blue)] transition delay-50 duration-150 ease-in-out hover:scale-110 text-xs md:text-sm lg:text-lg"><span className="mr-2"><FontAwesomeIcon icon={faDollarSign} /></span>Buy</Link>

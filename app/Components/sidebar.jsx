...
import { CartContext, LoginContext } from "../layout";
import Link from "next/link";
import { usePathname } from "next/navigation";
export default function Sidebar() {
    const {cart,setCartContext} = useContext(CartContext);
    ...
    return (
        <nav className="hidden flex-col w-3/15 h-screen md:flex md:bg-[var(--oxford-blue)] fixed">
            <ul className="flex flex-col my-5 mx-5">
                ...
                <Link href={`/cart/${userid}`}><li className="px-4 hover:bg-[var(--yale-blue)]/80 py-3 rounded-2xl">
                ...
                {
                    isLogin ? <Link href="/auth/logout"><li className="px-5 hover:bg-[var(--yale-blue)]/80 py-3 rounded-2xl"><span className="mr-2"><FontAwesomeIcon icon={faRightFromBracket} size="lg" /></span> Logout</li></Link>
                    : null
                }
            </ul>
        </nav>
